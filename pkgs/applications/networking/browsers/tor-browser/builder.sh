...
# fixup the firefox executable.  paxctl + patchelf tends to create invalid ELF
# headers; thus we only apply the PaX markings and set the dynamic library path
# and interpreter in the wrapper script.
...
#
# it is unnecessary to manipulate the python search path: the script adds its
...
#
# have to be somewhat specific here, not all transport plugins are statically
# linked ELFs, some may be Python scripts (e.g., fteproxy).
sed \
...
HOME=$TMPDIR TBB_DEBUG=1 $out/bin/$pname --help

torrc=$out/share/tor-browser/Browser/TorBrowser/Data/Tor/torrc-defaults
HOME=$TMPDIR $out/share/tor-browser/Browser/TorBrowser/Tor/tor -f "$torrc" --help
